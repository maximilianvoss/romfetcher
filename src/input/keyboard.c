...
static void renderSearchField(app_t *app) {
    ...
    if (*(app->keyboard.text) != '\0') {
        texture_t texture;
        rendering_loadText(app->sdlRenderer, &texture, app->keyboard.text, getActiveTheme(app)->fonts.font24,
    ...
    if (app->keyboard.activeChar != '\0') {
        ...
        texture_t texture;
        rendering_loadText(app->sdlRenderer, &texture, miniString, getActiveTheme(app)->fonts.font24,
...
static void renderKey(app_t *app, int posx, int posy, int padWidth, int padHeight, uint8_t active, char *text) {
    ...
    if (text != NULL && *text != '\0') {
        texture_t texture;
        rendering_loadText(app->sdlRenderer, &texture, text, getActiveTheme(app)->fonts.font26,
                           themes_getDrawColor(app, keyboardText));
